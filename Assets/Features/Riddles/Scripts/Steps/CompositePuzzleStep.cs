...
namespace DioramaEnigma.Riddles
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "Riddles/Steps/Composite Step", fileName = nameof(CompositePuzzleStep))]
    public sealed class CompositePuzzleStep : IdentifiableObject, IPuzzleStep
    {
        ...
        protected override void OnValidate()
        {
            ...
            foreach (var child in children)
                if (child != null && child is not IPuzzleStep)
                    ServiceDebug.LogWarning(this, $"Дочерний объект «{child.name}» не является шагом (IPuzzleStep)");
        }
#endif
        ...
        private IEnumerable<IPuzzleStep> EnumerateSteps()
        {
            if (children == null) yield break;

            foreach (var child in children)
                if (child is IPuzzleStep step)
                    yield return step;
        }
